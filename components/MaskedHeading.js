...
export default function MaskedHeading({
  ...
  // "Latest ref" pattern: keeps a mutable snapshot for the imperative RAF loop
  // below without re-subscribing effects on every prop change.
  const settingsRef = useRef({});
  // eslint-disable-next-line react-hooks/refs
  settingsRef.current = { fillScale, parallax, drift, brightness, saturation, grayscale, duotone, textScale };
  ...
  const place = useCallback(() => {
    ...
    const toneFilter = s.grayscale
      ? " grayscale(1) contrast(1.05)"
      : s.duotone
        ? " sepia(0.45) saturate(220%) hue-rotate(62deg) contrast(1.05)"
        : "";
    media.style.filter = `brightness(${s.brightness}) saturate(${s.saturation})${toneFilter}`;
  }, []);
